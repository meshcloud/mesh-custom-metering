...
class MeshStackClient:
    ...
    def get_tenants(
        self, platform_id: str, page_size: int = 100, include_deleted: bool = True
    ) -> Dict:
        ...
        logging.debug(
            f"Fetching tenants for platform {platform_id}, URL: {url}, include_deleted: {include_deleted}"
        )
        ...
        try:
            while True:
                ...
                response = requests.get(
                    url, headers=self.meshfed_headers, auth=self.auth, params=params
                )
                logging.debug(
                    f"MeshStack tenants API response status: {response.status_code}"
                )
                ...
                for tenant in tenants:
                    ...
                    if local_id:
                        ...
                        status_msg = " (DELETED)" if deletion_info else ""
    ...
    def _submit_usage_report(
        self, platform_tenant_id: str, date: str, payload: Dict
    ) -> Dict:
        ...
        try:
            response = requests.put(
                url, headers=self.kraken_headers, auth=self.auth, json=payload
            )
            logging.debug(
                f"MeshStack submit report API response status: {response.status_code}"
            )
    ...
    def submit_usage_report(
        ...
        try:
            ...
            return {"status": "failure", "message": str(last_exception)}
        except Exception as e:
            logging.error(f"Unexpected error outside of retry: {e}")
            return {"status": "failure", "message": str(e)}


def prepare_payload(line_items: List[Dict], platform_id: str, source: str) -> Dict:
    ...
    logging.debug(
        f"Prepared payload with {len(line_items)} line items for platform {platform_id}"
    )
