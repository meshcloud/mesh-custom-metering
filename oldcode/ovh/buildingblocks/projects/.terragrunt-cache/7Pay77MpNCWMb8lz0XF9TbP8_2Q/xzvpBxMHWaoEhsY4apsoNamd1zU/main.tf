...
# Shared Account Payment-Method Read for all (Admin, User, Reader)
resource "ovh_iam_policy" "account_payment_read" {
  count       = length(local.admin_urns) + length(local.user_urns) + length(local.reader_urns) > 0 ? 1 : 0
  name        = "${var.workspace_id}-${var.project_id}-account-payment-read"
  description = "Allow viewing payment methods for all roles"
  identities  = concat(local.admin_urns, local.user_urns, local.reader_urns)
  resources   = ["urn:v1:eu:resource:account:${var.ovh_account_id}"]
